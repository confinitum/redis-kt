...
suspend fun Redis.bitop(op: RedisBitop, destKey: String, vararg srcKeys: String): Long =
    executeTyped("BITOP", op.name, destKey, *srcKeys)
 ...
 * @since 2.8.7
 */
suspend fun Redis.bitpos(key: String, bit: Int, byteRange: LongRange?): Long =
    executeTyped("BITPOS", key, bit, *arrayOfNotNull(
        byteRange?.start,
        byteRange?.endInclusive
    )
    )
 ...
 * @since 1.0.0
 */
suspend fun Redis.getset(key: String, value: String): String? = executeTypedNull<String>("GETSET", key, value)
...
suspend fun Redis.msetnx(vararg pairs: Pair<String, String>): Boolean =
    executeTyped("MSETNX", *pairs.flatMap { listOf(it.first, it.second) }.toTypedArray())
...
suspend fun Redis.msetnx(map: Map<String, String>): Boolean =
    executeTyped("MSETNX", *map.flatMap { listOf(it.key, it.value) }.toTypedArray())
